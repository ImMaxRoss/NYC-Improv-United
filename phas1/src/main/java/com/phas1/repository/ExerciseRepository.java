...
import com.phas1.model.Exercise;
...
public interface ExerciseRepository extends JpaRepository<Exercise, Long> {
    ...
    // Find public exercises or exercises created by a specific coach
    @Query("SELECT e FROM Exercise e WHERE e.isPublic = true OR e.createdBy = :coach")
    ...
    @Query("SELECT DISTINCT e FROM Exercise e JOIN e.focusAreas fa " +
           "WHERE (e.isPublic = true OR e.createdBy = :coach) AND fa.id IN :focusAreaIds")
    ...
    @Query("SELECT DISTINCT e FROM Exercise e LEFT JOIN e.focusAreas fa " +
           "WHERE (e.isPublic = true OR e.createdBy = :coach) " +
