...
import com.backend.dto.LessonResponse.LessonExerciseResponse;
...
public class LessonController {
	...
	@Operation(summary = "Create a new lesson template", description = "Create a reusable lesson template without scheduling it as a lesson")
	@ApiResponses({
		@ApiResponse(responseCode = "201", description = "Template created successfully"),
		@ApiResponse(responseCode = "400", description = "Invalid request data"),
		@ApiResponse(responseCode = "401", description = "Not authenticated")
	})
	public ResponseEntity<LessonResponse> createTemplate(
			@Valid @RequestBody LessonRequest request,
			Authentication authentication) {
		
		Coach coach = userService.getCurrentCoach(authentication.getName());
		
		// Ensure the request is marked as a template
		request.setTemplate(true);
		
		// Map request to entity (similar to createLesson but without scheduling)
		Lesson template = new Lesson();
		template.setName(request.getName());
		template.setWorkshopType(request.getWorkshopType());
		template.setIsTemplate(true); // Explicitly set as template
		
		// Set team if provided (optional for templates)
		if (request.getTeamId() != null) {
			Team team = teamService.getTeamIfOwnedByCoach(request.getTeamId(), coach);
		...
		if (request.getExercises() != null && !request.getExercises().isEmpty()) {
			...
			template.setExercises(lessonExercises);
		}
		
		Lesson savedTemplate = lessonService.createLesson(template, coach);
		
		return ResponseEntity.status(HttpStatus.CREATED)
				.body(mapToLessonResponse(savedTemplate));
	}

    // ===== TEMPLATE UPDATE =====
    @PutMapping("/templates/{templateId}")
    @Operation(summary = "Update a lesson template", description = "Update an existing lesson template")
    @ApiResponses({
    ...
    public ResponseEntity<LessonResponse> updateTemplate(
            @PathVariable Long templateId,
            @Valid @RequestBody LessonRequest request,
            Authentication authentication) {
        ...
        // Verify the lesson is a template
        Optional<Lesson> template = lessonService.getLessonById(templateId, coach);
        if (template.isEmpty() || !template.get().isTemplate()) {
            return ResponseEntity.notFound().build();
        }
        
        // Proceed with update
        return updateLesson(templateId, request, authentication);
    }

    // ===== TEMPLATE DELETE =====
    @DeleteMapping("/templates/{templateId}")
    @Operation(summary = "Delete a lesson template", description = "Delete an existing lesson template")
    @ApiResponses({
    ...
    public ResponseEntity<Void> deleteTemplate(
        ...
        return deleteLesson(templateId, authentication);
    }
