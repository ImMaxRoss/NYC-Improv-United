## Quick Start
- Base URL: `http://localhost:8080` (dev) / `https://api.prod.com` (prod)
- Auth: Get token from `/api/auth/login` → add to `Authorization: Bearer {token}`
- Rate Limits: 100 requests/minute

### Test Users
| Role  | Email           | Password  |
|-------|-----------------|-----------|
| Coach | johndoe@gmail.com  | password123   |


### Database Schema / Sample Population Script
```sql
-- Schema.sql
...
('One Word Story', 'Players create a story one word at a time, enhancing listening skills and collaboration.', 10, NULL, true, NOW(), NOW()),
('Park Bench', 'Scene game where one player must justify why they leave the bench when a new character enters.', 7, NULL, false, NOW(), NOW()),
('Emotion Orchestra', 'Coach conducts players to express different emotions with sound and motion.', 8, NULL, true, NOW(), NOW()),
('Character Walks', 'Players explore characters physically through exaggerated walking styles.', 6, NULL, true, NOW(), NOW()),
('Mirroring', 'Two players mirror each other’s movements and facial expressions to improve nonverbal communication.', 5, 1, false, NOW(), NOW()),
('Word Association Circle', 'Quick word association around a circle to promote fast thinking and connections.', 5, 1, true, NOW(), NOW()),
('Scene Painting', 'Descriptive exercise where players collaboratively describe and imagine a setting.', 10, 1, true, NOW(), NOW()),
('Gibberish Translator', 'One player speaks in gibberish while the other translates, promoting teamwork and commitment.', 6, 1, true, NOW(), NOW()),
('Genre Replay', 'Perform the same scene multiple times in different film or TV genres.', 12, 1, true, NOW(), NOW());
```